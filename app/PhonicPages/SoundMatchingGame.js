...
export default function SoundMatchingGame({ route, onBack, onComplete }) {
  ...
  const handleOptionPress = (selectedLabel) => {
    ...
    setUserAnswers((prev) => [
      ...prev,
      {
        question: currentQuestion.correctAnswers[0].label,
    ...
    if (correct) {
      ...
      Speech.speak(
        `Wrong! This is ${selectedLabel}, not a word starting with`,
        {
          language: "en-US",
          pitch: 1.0,
          rate: 0.9,
  ...
  // ✅ Save final score + answers to Firestore
  const handleGameComplete = async (finalScore) => {
    ...
    const data = {
      ...
      percentage,
      completed: true,
      answers: userAnswers, // log all attempts
    };

    // Store in LetterSoundsScores
    await storeGameScore(null, "SoundMatchingGame", data);

    // Store in UserProgress
    await logUserProgress(null, "SoundMatchingGame", data);

    Speech.speak("Great job! You finished.", { language: "en-US" });
    setFeedbackMessage(`🎉 Final Score: ${finalScore}`);
    ...
    <View style={styles.container}>
      <View style={styles.headerRow}>
        ...
        <Text style={styles.heading}>
          Sound Matching - Letter {targetLetter}
        </Text>
      </View>
...
const styles = StyleSheet.create({
  ...
  correctFeedback: { backgroundColor: "rgba(76, 175, 80, 0.9)" },
  wrongFeedback: { backgroundColor: "rgba(244, 67, 54, 0.9)" },
});
